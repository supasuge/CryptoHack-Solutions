...
The **Gram–Schmidt** algorithm constructs an **orthogonal** set of vectors $u_1, u_2, u_3, u_4$ such that for each $u_{i}$, the span of $\{u_1, \dots, u_i\}$ is the same as the span of $\{v_1, \dots, v_i\}$. Concretely, the algorithm goes as follows (indexing $i=1$ to $4$ in our case):
...
3. Subtract this projection from $v_2$:
...
5. $\|u_2\|^2 = u_2 \cdot u_2 \approx 29.41753$ (we’ll use this later).
...
$$
\text{Volume} = |\det(A)| = |-255| = 255.
$$

Hence, the volume of the fundamental domain is **255**.
